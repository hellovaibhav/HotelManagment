...
const SideNavbar = () => {
    ...
    return (
        <div className={`w-full z-50 border-r-2 border-purple ${sideNavToggle === false ? " max-md:-translate-y-[50rem]" : "  max-md:translate-y-0"} md:w-[30%] backdrop-blur-md p-2 transition-all  duration-[2s] absolute md:sticky left-0 top-0 h-[60vh] md:h-screen`}>
            ...
            <div className='flex flex-col justify-center items-end gap-4 text-2xl h-full text-grey'>
                ...
                <Link to={'/bookings'} onClick={() => setSideNavToggle(!sideNavToggle)} className={`btn ${pathname === '/bookings' ? "bg-lightSecondary text-white" : ""}`}>Bookings</Link>
            </div>
        </div>
    )
}
export const ToggleNav = () => {
    let { sideNavToggle, setSideNavToggle } = useContext(ToggleSideNavbarContext)
    return (
        <div className='flex justify-end p-1 px-4 max-md:cursor-pointer' onClick={() => setSideNavToggle(!sideNavToggle)}>
